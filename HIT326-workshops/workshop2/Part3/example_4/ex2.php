...
<h2>SELECT Name, city, timezone FROM airports WHERE country = "Australia"</h2>
...
try{
  ...
  //Here is the prepared statement
   if($statement = $db->prepare("SELECT name, city, timezone FROM airports WHERE country = ?")){

      //The values in this array are bound to the question mark in the prepared SELECT
      $binding = array('Australia');
