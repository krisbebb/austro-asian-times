...
<input type='text' id='tocheck' name='tocheck'
<?php
  if(!empty($_GET['tocheck'])){
    echo 'value="'.$_GET['tocheck'].'"';
  }
?>
...
<?php
  if(!empty($_GET['tocheck'])){
     ...
     if($times =  @preg_match('/^(https?):\/\/(www\.example\.com(\.au)?)\/\?(name=([a-z0-9]+)&(profile|photo|friends))$/i',$_GET['tocheck'],$matches)){
...
<?php
 //Where does the array $matches come from?
 //Check PHP documentation for the preg_match function!
 if(!empty($matches) && is_array($matches)){
    echo "<h2>Captures</h2>";
	echo "<ul>";
    foreach($matches As $index => $match){
	     echo "<li>[{$index}] = {$match}</li>";
	}
	echo "</ul>";
 }
